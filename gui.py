...
with dpg.window(tag="MAIN"):
    ...
    with dpg.collapsing_header(label="Runner", default_open=True):
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(label="Run",  width=110, height=30, callback=lambda: [runner.run(runner.get_dpg_values()), runner.display_results()])
...
dpg.create_viewport(title='KrakenSploit', width=900, height=800, small_icon="addons/krak&logo.ico", large_icon="addons/krak&logo.ico")
dpg.setup_dearpygui()
