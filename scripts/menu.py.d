...
def scan_network_devices_and_services():
    ...
    # sc = nmap.PortScanner()
    # sc.scan('10.33.52.21' +'/'+str(32), arguments="-Pn -sV -O")
    print('Enter the target IP address or the target network (Example: 192.168.1.1/24)')
    scnet = input('Enter your choice: ')
    if validate_ip(scnet):
        sc = nmap.PortScanner()
        sc.scan(scnet, arguments="-Pn -sV -O")
    else:
    ...
    for h in sc.all_hosts():
        ...
        if 'osmatch' in sc[h]:
            print('OS Detection: %s' % ', '.join([match['name'] for match in sc[h]['osmatch']]))
        else:
